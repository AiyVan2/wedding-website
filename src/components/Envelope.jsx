...
function EnvelopePage({ onNext }) {
  ...
  // SVG viewport: 600 wide, 380 tall
  // Envelope body: y=0 to y=280 (the rect)
  // Flap triangle: points top-left(0,0) top-right(600,0) bottom-center(300,180)
  // Side folds: left and right triangles inside the body
  // Bottom fold: bottom triangle inside the body
  // Letter: absolutely positioned inside the wrapper, slides up

  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit"
      style={{
        display: "flex", flexDirection: "column", alignItems: "center",
        justifyContent: "center", minHeight: "100vh", gap: 24, padding: 32,
        background: "#fdf6ee",
      }}>
      ...
      {/* Wrapper — gives the SVG a real layout box so the letter can be positioned inside */}
      <div
      ...
      >
        {/* All envelope chrome is pure SVG — zero CSS triangle tricks */}
        <svg
          viewBox="0 0 600 380"
          width="100%"
        ...
        >
          {/* Body background */}
          <rect x="0" y="100" width="600" height="280" rx="8" fill="#c8976a" />

          {/* Left side fold */}
          <polygon points="0,100 0,380 300,240" fill="#b5835a" />

          {/* Right side fold */}
          <polygon points="600,100 600,380 300,240" fill="#b5835a" />

          {/* Bottom fold */}
          <polygon points="0,380 600,380 300,240" fill="#d4a574" />

          {/* Flap — sits on TOP of body, hinges from the top edge (y=100) */}
          <motion.polygon
            points="0,100 600,100 300,280"
            fill="#e76f51"
            animate={{ rotateX: open ? 180 : 0 }}
            transition={{ duration: 0.85, ease: [0.22, 1, 0.36, 1] }}
            style={{ transformOrigin: "300px 100px", transformBox: "fill-box" }}
          />
        </svg>

        {/* Letter card — lives in normal flow, absolutely on top of SVG */}
        <motion.div
          ...
          style={{
            ...
            zIndex: open ? 10 : 2,
            pointerEvents: "none",
            boxShadow: "0 2px 16px rgba(92,45,30,0.08)",
          }}
        >
          <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 11, letterSpacing: 3, color: "#c9956b", textTransform: "uppercase" }}>
            you are cordially invited
          </span>
          <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(20px,3.5vw,30px)", color: "#5c2d1e", fontStyle: "italic" }}>
            You are Invited!
          </span>
          ...
          {revealed && (
            <motion.button
              ...
              onClick={onNext}
              style={{
                marginTop: 6, padding: "9px 26px",
                background: "#5c2d1e", color: "#fffdf7",
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit"
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(36px,8vw,72px)", color: "#5c2d1e", fontStyle: "italic", lineHeight: 1.1, marginBottom: 8 }}>
          {WEDDING.bride}
        </h1>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(22px,5vw,40px)", color: "#c9956b", marginBottom: 8 }}>&amp;</p>
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(36px,8vw,72px)", color: "#5c2d1e", fontStyle: "italic", lineHeight: 1.1, marginBottom: 32 }}>
          {WEDDING.groom}
        </h1>
